    ...
    <main>
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.manage_surah_attachments") }}</h5>
        ...
        <div class="title_route_wrapper" v-if="$can('surahs create', 'surahs')">
          <router-link to="/addition-surah/create">
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== '1') {
        await this.$router.push({ path: '/addition-surah/all', query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
      ...
      try {
        let res = await this.$axios({
          ...
          params: {
            ...
            search: this.filterOptions.select_surahs?.name,
    ...
    async changeActivationStatus(item) {
      try {
        await this.$axios({
          method: "POST",
          url: `addition_surahs/toggle/${item.id}`,
        });
        this.setTableRows();
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/addition-surah/edit/${item.id}` });
    },

    showItem(item) {
      this.$router.push({ path: `/addition-surah/show/${item.id}` });
    },
    // ===== End:: End
