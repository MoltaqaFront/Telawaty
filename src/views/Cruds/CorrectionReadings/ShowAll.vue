    ...
    <main>
        ...
        <div class="filter_form_wrapper">
          <form @submit.prevent="submitFilterForm">
            <div class="row justify-content-center align-items-center w-100">
              ...
              <base-input col="3" type="text" :placeholder="$t('PLACEHOLDERS.word')" v-model.trim="filterOptions.word" />
              ...
              <base-input col="3" type="number" :placeholder="$t('PLACEHOLDERS.verse_number')"
                v-model.trim="filterOptions.verse_number" />
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.manage_recitation_correction") }}</h5>
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.created_at`]="{ item }">
          <p class="text-danger" v-if="!item.created_at"> - </p>
  ...
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: this.$t("PLACEHOLDERS.verse_number"),
          value: "aya_id",
          sortable: false,
          align: "center",
        },
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== '1') {
        await this.$router.push({ path: '/correction-reading/all', query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
      this.filterOptions.select_surahs = null;
      this.filterOptions.word = null;
      this.filterOptions.verse_number = null;
      ...
      try {
        let res = await this.$axios({
          ...
          params: {
            ...
            ayah_text: this.filterOptions.word,
            aya_id: this.filterOptions.verse_number,
    ...
    async changeActivationStatus(item) {
      try {
        await this.$axios({
          method: "POST",
          url: `correctionReading/toggle/${item.id}`,
        });
        this.setTableRows();
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/correction-reading/edit/${item.id}` });
    },

    showItem(item) {
      this.$router.push({ path: `/correction-reading/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `correction-reading/${this.itemToDelete.id}`,
        });
