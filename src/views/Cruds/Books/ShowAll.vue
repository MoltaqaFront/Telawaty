    ...
    <main>
        ...
        <div class="filter_form_wrapper">
          <form @submit.prevent="submitFilterForm">
            <div class="row justify-content-center align-items-center w-100">
              ...
              <!-- Start:: Name Input -->
              <base-input col="6" type="text" :placeholder="$t('PLACEHOLDERS.book_name')"
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.library_management") }}</h5>
        ...
        <div class="title_route_wrapper" v-if="$can('books create', 'books')">
          <router-link to="/books/create">
            {{ $t("PLACEHOLDERS.add_new_book") }}
          </router-link>
        </div>
      </div>
      <!--  =========== End:: Table Title =========== -->
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <div class="actions">
            ...
            <a-tooltip placement="bottom" v-if="$can('books show', 'books')">
              <template slot="title">
            ...
            <a-tooltip placement="bottom" v-if="$can('books edit', 'books')">
              <template slot="title">
            ...
            <a-tooltip placement="bottom" v-if="$can('books delete', 'books')">
              <template slot="title">
  ...
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: this.$t("PLACEHOLDERS.book_name"),
          value: "name",
          sortable: false,
          align: "center",
        },
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== '1') {
        await this.$router.push({ path: '/books/all', query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/books/edit/${item.id}` });
    },

    showItem(item) {
      this.$router.push({ path: `/books/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `books/${this.itemToDelete.id}`,
        });
